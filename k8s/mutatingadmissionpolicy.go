...
var (
	errNoObject    = errors.New("an object is required to evaluate mutations: paste the object the policy should act on into the Object tab")
	errNoObjectGVK = errors.New("the object must set apiVersion and kind to be mutated, so the Object tab needs both before a schema can be found for it")
)
...
//     outright. A mutation whose output grows past maxMutatedObjectBytes is
//     reported the same way, and the mutations after it run against the object
...
func evalMutations(policy *admissionregistrationv1.MutatingAdmissionPolicy, oldObjectInput, objectInput, namespaceInput, requestInput []byte, authorizer *Authorizer) (*mutationRun, error) {
	...
	// Rebuild what the apiserver's unexported compilePolicy does
	// (k8s.io/apiserver pkg/admission/plugin/policy/mutating/compilation.go), from the
	...
	for _, mutation := range policy.Spec.Mutations {
		...
		if len(mutatedYAML) > maxMutatedObjectBytes {
			// Checked before the schema probes below, which walk the whole
			// document several times: there is nothing to learn about an
			// object that is not going to be applied.
			//
			// The copy limit armed in init bounds one patch; nothing bounds
			...
			setMutationError(result, fmt.Errorf(
				"the mutated object is %d bytes, past the %d a cluster accepts in a request "+
					"body, so it was not applied or passed to the mutations after it",

...
func TestMutationEval(t *testing.T) {
	tests := []struct {
		...
		// A patch that copies part of the object into itself doubles it each
		// time. Nothing in the CEL budget prices that, so the only thing
		// between a policy and a browser tab is the size the result is allowed
		// to reach.
		name:   "a patch that grows the object past what a cluster would store",
		policy: "copy grows the object policy.yaml",
		object: "object.yaml",
		mutations: []mutationExpectation{{
			patchType:     "JSONPatch",
			isError:       true,
			errorContains: "past the 1048576 a cluster accepts in a request body",
		}},
		wantNoFinal: true,
		wantDiff:    false,
	}, {
		// Further still, and the patch library refuses it while applying,
		// which is the limit a real apiserver arms at startup. The message is
		// upstream's own.
		name:   "a patch that copies past the limit a cluster sets",
		policy: "copy exceeds the patch limit policy.yaml",
		object: "object.yaml",
		mutations: []mutationExpectation{{
			patchType:     "JSONPatch",
			isError:       true,
			errorContains: "the accumulated size increase of copy",
		}},
		wantNoFinal: true,
		wantDiff:    false,
	}, {
...
// exists to answer.
// TestAVariableThatReadsItselfIsAnError covers an expression that resolves its
// own name. The value is recorded only once the expression returns, so without
// a guard the activation resolves the name by evaluating it again, unbounded,
// until the stack runs out and the whole module dies -- taking every later run
// in the session with it. Kubernetes refuses the policy at creation, so there
// is no cluster behaviour to reproduce here, only a crash to avoid.
func TestAVariableThatReadsItselfIsAnError(t *testing.T) {
	object := []byte("apiVersion: apps/v1\nkind: Deployment\nmetadata:\n  name: nginx\n")
	...
	tests := []struct {
		...
		name: "a variable that reads itself",
		variables: `
  - name: a
    expression: "variables.a"`,
	}, {
		// The second declaration replaces the first, so its read of
		// variables.a resolves to the entry being defined.
		name: "two variables with one name",
		variables: `
  - name: a
    expression: "'x'"
  - name: a
...
spec:
  ...
  validations:
  - expression: "string(variables.a) != ''"
`)
			out, err := k8s.EvalValidatingAdmissionPolicy(policy, nil, object, nil, nil, nil)
			if err != nil {
			...
			if len(response.ValidationVariables) == 0 {
				t.Fatal("no variables reported")
			}
			last := response.ValidationVariables[len(response.ValidationVariables)-1]
			if !last.IsError {
				t.Errorf("variable %s = %v, want an error rather than a value",
